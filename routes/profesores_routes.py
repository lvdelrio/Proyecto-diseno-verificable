...
def view_profesor(profesor_id):
    ...
    return render_template("Profesor/detalle_profesor.html", profesor=profesor)

@profesor_route_blueprint.route('/agregar_profesor', methods=['POST'])
def add_profesor():
    nombre = request.form.get("nombre")
    descripcion = request.form.get("descripcion", "")
    profesor = crear_profesor(config.session, nombre, descripcion)
...
def edit_profesor(profesor_id):
    ...
    profesor = edit_profesor_by_id(config.session, profesor_id, name, description)
